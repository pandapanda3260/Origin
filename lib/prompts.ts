...
⚠️【关键约束 - 镜头数量】
  ...
  · 多角色对白（>3 句）允许用一个"群像反应中景"镜头一镜到底承载，不要拆成 N 个反应镜头
  · 每个 duration 默认 3-5 秒（合并镜头可到 5-6 秒），整片总时长贴近用户给的目标值

⚠️【关键约束 - 单镜头只做一个核心运镜，不要叠加】
  · camera 字段每个镜头**只能写一种核心运镜方式**（从下方列表里选一个），不要写"先推近再环绕"、
    "缓慢推进 + 跟随" 这种复合描述
  · 一个 5 秒镜头的注意力预算非常有限——一个核心运镜动作就够讲完一个画面意图，再叠加运镜
    会让观众抓不住重点也会让模型生成时画面糊
  · 整片可以用多种不同运镜（不限种类）—— 词汇库丰富才能精准表达情绪，不强制全片只用 3-5 种
  · 但相邻 2-3 个镜头之间运镜要"自然衔接"：能同向同焦段最好（比如连续 2 个"轻微推近"），
    不要在相邻镜头之间硬切运镜方向（比如刚"快速推进"下一个就"快速拉远"——除非剧情真有这种翻转节奏）
  · 推荐倾向（不强制，可按需偏离）：
      setup    → 固定镜头 / 缓慢推进 类（建立环境）
      rising   → 轻微推近 / 推近 / 跟随 类（拉近距离）
      climax   → 快速推进 / 手持轻晃 / 甩镜头 类（情绪爆发）
      falling  → 缓慢拉远 / 拉远 类（情绪释放）
      resolution → 缓慢拉远 / 固定镜头 类（收束）
...
【整体规则】
  ...
  · **总镜头数控制在 6-12 个**，目标总时长贴近用户给的值（默认 30-50 秒成片）
  · 关键剧情节点（开场环境、人物登场、冲突爆发、转折、收尾）各自一个独立镜头即可，不要为同一节点拆多个反应镜头
  · 同一场景内的多句对白可以一镜到底（群像中景）承载，不强制每句拆一镜
  · **camera 字段单镜头只能写一种核心运镜**——禁止"缓慢推进+轻微环绕"、"先推近再拉远"这种叠加；不同镜头之间运镜手法可以丰富多样

【自检 - 输出前请逐条对照】
  · 镜头总数是否 ≤ 12？>12 必须合并冗余反应镜头
  · 每个 camera 字段是否只有**一个**运镜词（来自下方推荐列表的单项）？发现复合的（带 + / 加 / 然后 / 再 / 接着 这种连接词）必须拆掉只留主导那一个
  · 相邻镜头 camera 是否衔接顺畅？出现剧烈反向运镜（如刚"快速推进"立刻"快速拉远"）且无剧情翻转支撑的，必须改其中一个为同向缓动
  · visual 字数 < 50 必须补细节
  · keyInfo 超过 8 字必须缩短
  · camera 只写了"推镜头""拉镜头"没有速度修饰必须改为"推近"或"缓慢推进"
...
export function buildVideoPromptMessages(opts: {
  ...
  parts.push(
    `本组镜头（共 ${slimShots.length} 个，按时间顺序）：\n${JSON.stringify(slimShots, null, 2)}\n\n` +
      `⚠️ 本组每个镜头的核心运镜方向 / 景别（**视觉标签 ⟦…⟧ 和运镜系统段的方向必须忠于这里——推就是推、拉就是拉、静就是静；可以用"轴线/焦段/平稳缓推/平移跟拍"等更专业的词把它展开描述，但禁止改方向，也禁止一个镜头里叠加多个运镜动作**）：\n` +
      cameraList,
  );
