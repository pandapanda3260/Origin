...
test('guardMode off allows dialogue sanitization but still rejects structural drift', () => {
  const facts = factsForRefineGuardMode({
    dialogueTexts: ['杀了他'],
    timeRangeTitles: ['0-3s'],
    imageNumbers: [1],
    characterNamesOrIds: ['阿岚'],
    referenceLabels: ['角色参考'],
  }, 'off');
  ...
  assert.equal(validateRefineOutput({
    originalPrompt,
    refinedPrompt: dialogueOnly,
    facts,
  }).accepted, true);

  const changedImage = dialogueOnly.replace('Image 1', 'Image 2');
  const result = validateRefineOutput({
    originalPrompt,
    refinedPrompt: changedImage,
    facts,
  });
  assert.equal(result.accepted, false);
  assert.deepEqual(result.violations.map((item) => item.type), ['image_number_changed']);
});
