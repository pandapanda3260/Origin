...
- **后端**：分 5 阶段做（详见 [BACKEND_PHASES.md](./BACKEND_PHASES.md)）
  ...
  - ✅ **阶段五**（已完成）：积分扣减/退还 + 兑换码 + 管理面板真数据 + 系统日志 + 维护横幅 + Stripe 脚手架
...
| 套餐积分 / 兑换码 (`/api/billing/*`) | ✅ 真后端（积分扣减 + 退还 + 兑换码 + Stripe 脚手架） |
| 管理面板统计 (`/api/auth/admin/*`) | ✅ 真后端（真 DB 查询 + 系统日志环形缓冲） |
| 维护横幅 / Key 池 / 配置中心 (`/api/maintenance/*`、`/api/admin/*`) | ✅ 真后端（admin PUT 可写） |
| 其他兜底接口 | ⏳ 兜底返回空数据（生产前再细做） |
