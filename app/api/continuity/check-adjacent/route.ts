      ...
      try {
        const json = await chatCompleteJsonWithRetry<any>(
          ...
          ],
          {
            ...
            tokenContext: {
              projectId,
              projectTitleSnapshot: cleanStr(project?.title, project?.name),
              requestPath: req.nextUrl.pathname,
              routeName: 'continuity.check-adjacent',
              moduleKey: 'continuity',
              moduleLabel: '连续性检查',
              featureKey: 'continuity.check-adjacent',
              featureLabel: '相邻镜头连续性检查',
              callItemType: 'project',
              callItemId: projectId,
              callItemLabel: cleanStr(project?.title, project?.name) || projectId,
              operationKey: `continuity:${projectId}`,
              operationLabel: '相邻镜头连续性检查',
            },
          },
          parseJsonLoose,
