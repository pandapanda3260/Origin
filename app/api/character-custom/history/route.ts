...
import { cleanupStaleCustomCharacterDrafts, listCustomCharacters } from '@/lib/custom-character-db';
...
export async function GET(req: NextRequest) {
  ...
  cleanupStaleCustomCharacterDrafts(user.id, 48);
  const items = listCustomCharacters({ ownerId: user.id, projectId, limit, lifecycleStatus });
