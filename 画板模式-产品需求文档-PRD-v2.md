...
3. **尾帧退出分镜 grid；首尾帧改为视频卡模式**（2026-06-25 更新，§7/§8）：分镜首帧 grid 默认无尾帧、不主动建议；**"首尾帧"作为视频卡的并列模式之一保留**（默认"全能参考"自动选最合适模式），用户选首尾帧时在视频卡提供结束帧；后端 `tail_frame_images`/`first_last_frame` 作该模式实现保留不删。
...
## 7. 尾帧策略（v3：分镜 grid 无尾帧；首尾帧 = 视频卡模式）
...
- **首尾帧 = 视频卡的一个模式（2026-06-25 更新，不再埋高级菜单）**：分镜首帧 grid 只管每镜头首帧候选、默认无尾帧/尾帧槽；A→B 转场/变身/物体变化改在**视频卡选「首尾帧」模式**时提供结束帧（§8/§9.1）。后端 `tail_frame`/`first_last_frame` 作该模式的正常实现保留。
...
- **视频卡模式选择器（默认全能参考，2026-06-25 Vasily 决策，对齐图5 模式 tab）**：视频 card 显式提供 **[全能参考(默认) | 智能多帧 | 首尾帧 | …]**——**全能参考**(默认)= `resolveVideoPayloadDecision` 按用户输入**自动选最合适模式**（即把原 auto 判断升格为默认用户模式，casual 无需选）；**智能多帧**= 上条关键帧+参考图（Seedance 2.5 多关键帧）；**首尾帧**= 显式转场、用户在视频卡提供结束帧，**不删不埋、并列模式之一**。`first_last_frame` payload 与 `tail_frame_images` 作"首尾帧"模式实现保留。最终模式集与各模式输入项在方案五（视频卡 UI）定稿。
...
- **模式暴露拍板（2026-06-25 更新，撤销"对用户无感/不暴露"）**：视频卡**显式提供模式选择器** **[全能参考(默认) | 智能多帧 | 首尾帧 | …]**，默认 **全能参考**=后端按输入自动选（casual 无需选），进阶可显式切 智能多帧/首尾帧。模式集与输入项在方案五定稿。
...
- **尾帧不出现在分镜 grid**；首尾帧作为视频卡模式之一可选（默认全能参考自动）。
...
| 方案四 · 视频生成模式（全能参考/智能多帧/首尾帧）升级 | 全能参考=auto 选；智能多帧=关键帧+参考（Seedance 2.5 30s/多关键帧/≤50）；首尾帧保留为模式；预算上调 | **Seedance 2.5 公测 GA（约 7 月初）** | 后端重头，GA 后落地 |
