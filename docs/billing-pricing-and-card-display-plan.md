...
| 积分包 | topup_500/2000/10000（¥19/¥69/¥299） | **topup_basic 基础包 4,000/¥100 · topup_advanced 进阶包 120,000/¥3,000 · topup_enterprise 企业包 2,000,000/¥40,000**（永久，进 topup 桶不过期；旧 code 直接删不留兼容） |
...
- `public/modules/billing.js`：
  ...
  - 积分包卡 meta：`¥100 · 4,000 积分 · 永久积分`（拍板文案"永久积分"）。
...
checkout 返回真实支付地址；fulfillPaidOrder 挪到回调验签后调用；renewDueSubscription 由续费扣款回调替代（开关关掉即停）；月末账期对齐网关；占位文案删除。兑换码种子（QDDEMO 最大 1 万分）量级与新价格体系（最小包 4 千分）上线前重定。
